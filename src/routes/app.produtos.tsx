...
function Produtos() {
  ...
  const handleDelete = async (id: string) => {
    if (!await confirm({ description: "Tem certeza que deseja excluir este produto?", variant: "destructive" })) return;
    try {
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">Carregando produtos...</TableCell>
                </TableRow>
              ) : filteredProducts.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">Nenhum produto encontrado na busca.</TableCell>
                </TableRow>
                ...
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-3">
                      <div className="grid h-10 w-10 overflow-hidden place-items-center rounded-lg bg-accent text-lg border">
                        {p.imagem ? <img src={p.imagem} alt={p.nome} className="w-full h-full object-cover" /> : <span className="opacity-50">{p.emoji || "📦"}</span>}
                      </div>
                      <div>
                  ...
                  <TableCell className="text-right font-semibold">{Number(p.estoque).toLocaleString("pt-BR")}</TableCell>
                  <TableCell className="text-right font-semibold">R$ {Number(p.valor).toFixed(2).replace('.', ',')}</TableCell>
                  <TableCell>
                    {p.status === "Ativo"
                      ...
                      : <Badge className="bg-muted text-muted-foreground hover:bg-muted/80 border-0">Inativo</Badge>}
                  </TableCell>
