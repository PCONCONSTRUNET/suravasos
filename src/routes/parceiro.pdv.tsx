        ...
        <div className="flex overflow-x-auto gap-2 pb-2 snap-x no-scrollbar sticky top-36 z-10 bg-slate-50 py-1">
          {categorias.map(cat => (
            <button
              ...
              className={`flex-shrink-0 snap-center px-4 py-2 rounded-full text-xs font-bold transition-colors ${selectedCategory === cat ? 'bg-emerald-700 text-white shadow-md' : 'bg-white text-slate-600 border border-slate-200'}`}
            >
              {cat}
            </button>
          ))}
