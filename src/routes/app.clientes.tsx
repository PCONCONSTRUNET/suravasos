...
function Clientes() {
  ...
  const handleDelete = async (id: string) => {
    if (!await confirm({ description: "Tem certeza que deseja excluir este cliente?", variant: "destructive" })) return;
    try {
  ...
  return (
    <>
      <PageHeader title="Clientes" subtitle={`${clientes.length} clientes cadastrados`} actions={
        <Button asChild className="bg-gradient-brand text-primary-foreground">
          <Link to="/app/cliente-novo"><Plus className="mr-2 h-4 w-4" />Novo Cliente</Link>
        </Button>
      } />
      ...
      <div className="grid gap-4 sm:grid-cols-3 mb-6">
        {[
          ...
          <Card key={String(l)} className="shadow-card"><CardContent className="p-5">
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader><TableRow>
              <TableHead>Nome / Razão Social</TableHead><TableHead>Documento</TableHead><TableHead>Cidade / UF</TableHead><TableHead>Telefone</TableHead>
            ...
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">Carregando clientes...</TableCell>
                </TableRow>
              ) : clientes.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">Nenhum cliente cadastrado ainda.</TableCell>
                </TableRow>
              ) : clientes.map((c) => (
                <TableRow key={c.id} className="cursor-pointer hover:bg-muted/50" onClick={() => handleOpenDetails(c)}>
                  <TableCell><div className="flex items-center gap-3">
                    <div className="h-9 w-9 rounded-full bg-gradient-brand grid place-items-center text-xs font-bold text-primary-foreground">
                      {c.nome.split(" ").map((x: string) => x[0]).slice(0, 2).join("").toUpperCase()}
                    </div>
                  ...
                  <TableCell className="text-muted-foreground text-sm">{c.ultima_compra ? new Date(c.ultima_compra).toLocaleDateString() : "-"}</TableCell>
                  <TableCell>
                    <Badge className={
                      c.status === "Premium" ? "bg-info/15 text-info hover:bg-info/20 border-0" :
                      c.status === "Ativo" ? "bg-success/15 text-success hover:bg-success/20 border-0" :
          ...
          <div className="mt-6 space-y-6">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="font-medium">{selectedCliente?.cidade ? `${selectedCliente.cidade}${selectedCliente.uf ? '/' + selectedCliente.uf : ''}` : "-"}</span>
              </div>
            ...
            <div className="border-t pt-6">
              ...
              {loadingVendas ? (
                ...
                <p className="text-sm text-muted-foreground">Nenhuma compra registrada para este cliente.</p>
              ) : (
                <div className="space-y-4">
                  {clientVendas.map(v => (
                    <div key={v.id} className="flex justify-between items-center p-3 rounded-lg border border-border/50">
                      <div>
                        <div className="font-semibold text-sm">Pedido #{v.id?.toString().slice(0, 5)}</div>
                        <div className="text-xs text-muted-foreground">{new Date(v.created_at).toLocaleDateString()}</div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-sm">R$ {Number(v.total || 0).toLocaleString('pt-BR', {minimumFractionDigits: 2})}</div>
                        <Badge variant="outline" className="text-[10px] mt-1">{v.status}</Badge>
                      </div>
                    </div>
                  ))}
                </div>
