...
function Catalogo() {
  ...
  const categoriasUnicas = ["Todos", ...Array.from(new Set(produtos.map(p => p.categoria))).filter(Boolean)] as string[];
  ...
  return (
    <>
      <PageHeader title="Catálogo Digital" subtitle="Compartilhe seus produtos pelo WhatsApp, link público ou QR Code" actions={
        <Button onClick={handleShareCatalog} className="bg-success text-success-foreground hover:bg-success/90">
          <WhatsAppIcon className="mr-2 h-4 w-4" />Compartilhar catálogo
        </Button>
      } />
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
        {loading ? (
          ...
          <p className="col-span-full text-center text-muted-foreground py-8">Nenhum produto encontrado na busca.</p>
        ) : filtrados.map((p, index) => (
          <Card key={p.id} className="overflow-hidden shadow-card hover:shadow-elevated transition-all group">
            <div className={`relative aspect-square overflow-hidden bg-gradient-to-br ${getGradient(index)} grid place-items-center text-7xl`}>
              ...
              {p.estoque < 10 && <Badge className="absolute top-3 left-3 bg-warning text-warning-foreground border-0">Últimas unidades</Badge>}
            </div>
            <div className="p-4">
              ...
              <div className="mt-2 flex items-baseline justify-between border-b pb-2 mb-2">
                <p className="text-primary font-display text-xl font-extrabold">R$ {Number(p.valor).toFixed(2)}</p>
              ...
              <div className="py-1 text-xs text-muted-foreground space-y-1 border-b pb-3 mb-3">
                {p.numero && <p><span className="font-semibold text-foreground">Número:</span> {p.numero}</p>}
                {p.dimensao && <p><span className="font-semibold text-foreground">Dimensões:</span> {p.dimensao}</p>}
                {p.volume && <p><span className="font-semibold text-foreground">Volume:</span> {p.volume} L</p>}
                {p.comprimento && <p><span className="font-semibold text-foreground">Comprimento:</span> {p.comprimento} cm</p>}
              ...
              <div className="mt-3 grid grid-cols-[1fr_auto] gap-2">
                <Button onClick={() => handleShare(p.nome)} size="sm" className="bg-success text-success-foreground hover:bg-success/90"><WhatsAppIcon className="mr-1.5 h-3.5 w-3.5" />WhatsApp</Button>
