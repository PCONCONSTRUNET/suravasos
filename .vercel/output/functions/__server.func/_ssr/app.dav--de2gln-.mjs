...
//#region node_modules/.nitro/vite/services/ssr/assets/app.dav--de2gln-.js
...
function DAVList() {
	...
	const fetchDAVs = async () => {
		try {
			const { data, error } = await supabase.from("vendas").select(`*, clientes (nome)`).eq("tipo", "DAV").order("created_at", { ascending: false });
